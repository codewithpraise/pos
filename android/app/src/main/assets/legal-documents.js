...
1. Local Database Storage: Offline transaction ledgers are stored locally using encrypted SQLite / IndexedDB engines.
2. Device Hardware Binding: Licenses bind to HWID hash values generated on initial register registration.
    `,
...
1. Telemetry Data: System telemetry is limited to storage capacity, version checksums, and diagnostic fault logs.
2. Customer Records: Customer names, phone numbers, and ledger entries are stored in your store's private database partition.
    `,

    ACCEPTABLE_USE: `
# VALENIXIA POS — ACCEPTABLE USE POLICY (v2.3.0)

1. Prohibited Actions: Reverse-engineering cryptographic license validation mechanisms or tampering with immutable FBR audit logs is strictly prohibited.
2. Rate Limits: Cloud sync services enforce fair-use bandwidth limits of 1000 sync batches per hour per terminal.
    `,

    FBR_DISCLAIMER: `
# VALENIXIA POS — FBR / FISCAL REGULATORY DISCLAIMER (v2.3.0)

1. Tax Rate Compliance: The merchant is responsible for configuring tax rates (e.g. 18% standard sales tax, 15% restaurant tax) in alignment with provincial tax authority mandates (FBR, PRA, SRB, BRA, KPRA).
2. Service Fee Collection: FBR POS service fee of Rs. 1.00 per invoice is collected where mandated for Tier-1 retailers.
    `,

    CLOUD_SYNC_TERMS: `
# VALENIXIA POS — CLOUD SYNC & DATA PROTECTION TERMS (v2.3.0)

1. Hybrid Logical Clocks: Multi-terminal conflict resolution relies on HLC timestamp ordering.
2. Zero-Loss Sync Queue: Transactions recorded offline are queued in SQLite outbox tables until connectivity is restored.
    `
  };

  global.LEGAL_DOCUMENTS = LEGAL_DOCUMENTS;
