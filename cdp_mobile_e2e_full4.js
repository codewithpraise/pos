...
function cdpEval(ws, script, id) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => { ws.removeListener('message', handler); resolve({ result: { value: 'EVAL_TIMEOUT' } }); }, 15000);
  });
}
...
http.get('http://localhost:9222/json', async (res) => {
  ...
  res.on('end', async () => {
    ...
    ws.on('open', async () => {
      ...
      await sleep(1000); // Small initial wait for page to start reloading

      // PHASE 1: License Activation  
      console.log('\n--- PHASE 1: License Activation ---');
      let r = await cdpEval(ws, `(async () => {
        // Poll for activation key field
        for (let i = 0; i < 80; i++) {
      ...
      // Wait for page reload to complete (which happens post-activation)
      await sleep(1000);
      ...
      r = await cdpEval(ws, `(async () => {
        // Wait for lock screen/pin pad to mount and activate
        for (let i = 0; i < 80; i++) {
          ...
          if (pad && pad.offsetHeight > 0 && document.getElementById('auth-lock-screen')?.classList.contains('active')) {
            ...
            clickDigit(3);
            clickDigit(4);
            // Verify clicks entered the value and verify Pin triggers
