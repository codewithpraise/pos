...
class EntitlementService {
  ...
  static async canAddBranch(organizationId, currentCountOverride) {
    const ent = (typeof organizationId === 'object' && organizationId !== null)
      ? organizationId
      : await this.getOrganizationEntitlements(organizationId);
  ...
  static async canAddTerminal(organizationId, currentCountOverride) {
    const ent = (typeof organizationId === 'object' && organizationId !== null)
      ? organizationId
      : await this.getOrganizationEntitlements(organizationId);
    const allowedLimit = ent.maxTerminals;
    let current = 0;
    ...
    if (current >= allowedLimit) {
      return {
        allowed: false,
        reason: `Terminal limit reached for ${ent.tier} plan (${current}/${allowedLimit}). Purchase an Extra Terminal add-on to register more devices.`,
        current,
        limit: allowedLimit
      };
    }
