...
class EntitlementService {
  ...
  static generateSignedOfflineSnapshot(payload) {
    ...
    const canonicalPayload = JSON.stringify(fullPayload, Object.keys(fullPayload).sort());
  ...
  static verifySignedOfflineSnapshot(snapshot, signature, publicKeyPem) {
    try {
      ...
      const canonicalPayload = JSON.stringify(snapshot, Object.keys(snapshot).sort());
  ...
  /**
   * Server-Authoritative Feature Gate Check
   */
  static async canUseFeature(organizationId, featureKey) {
    ...
    const effective = await this.getOrganizationEntitlements(organizationId);
    const hasFeature = Boolean(effective && effective.features && effective.features[featureKey]);

    if (!hasFeature) {
      return {
        allowed: false,
        reason: 'ADDON_LOCKED',
        message: `Feature entitlement '${featureKey}' is not active for this organization. Request activation from Subscription -> Add-ons.`
      };
    }
    return { allowed: true, entitlement: effective };
  }
