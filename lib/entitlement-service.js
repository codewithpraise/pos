...
class EntitlementService {
  ...
  static async canAddBranch(organizationId) {
    const ent = await this.getOrganizationEntitlements(organizationId);
    const allowedLimit = ent.maxBranches;
    const countRow = await db.get(
    ...
    if (current >= allowedLimit) {
      return {
        allowed: false,
        reason: `Branch limit reached for ${ent.tier} plan (${current}/${allowedLimit}). Purchase an Extra Branch add-on or upgrade plan.`,
        current,
        limit: allowedLimit
      };
    }
  ...
  static async canAddTerminal(organizationId, branchId) {
    const ent = await this.getOrganizationEntitlements(organizationId);
    const countRow = await db.get(
    ...
    if (current >= ent.maxTerminals) {
      return {
        allowed: false,
        reason: `Terminal limit reached for ${ent.tier} plan (${current}/${ent.maxTerminals}). Purchase an Extra Terminal add-on to register more devices.`,
        current,
        limit: ent.maxTerminals
      };
    }
    return { allowed: true, current, limit: ent.maxTerminals };
  }
