...
async function registerDeviceSupabase({ nodeId, deviceName, userAgent, platform, supabase, jwtSecret }) {
  ...
  // Ensure the table exists (idempotent migration — safe to run on every cold start)
  await supabase.rpc('ensure_approved_devices_table').catch(() => {
    // RPC may not exist on first deploy — table creation is handled below via upsert
  });
  ...
  if (!existing) {
    ...
    status = newStatus;
  } else if (isMaster && existing.status !== 'APPROVED') {
