...
function maskPII(str) {
  ...
  return str
    ...
    .replace(/("?(?:token|passphrase|secret|password|adminPin|jwt_secret|master_key|auth_hash|sync_salt)"?\s*[:=]\s*)"[^"]+"/gi, '$1"[REDACTED]"')
    .replace(/("?(?:token|passphrase|secret|password|adminPin|jwt_secret|master_key|auth_hash|sync_salt)"?\s*[:=]\s*)[^\s,}]+/gi, '$1[REDACTED]')
