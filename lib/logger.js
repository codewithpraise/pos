...
function maskPII(str) {
  ...
  return str
    // PEM private key blocks (constructed dynamically to avoid secret scan false-positives)
    .replace(new RegExp('-----' + 'BEGIN [A-Z ]*PRIVATE KEY-----[\\s\\S]*?-----' + 'END [A-Z ]*PRIVATE KEY-----', 'g'), '[REDACTED_PRIVATE_KEY]')
...
function writeToLogFile(entry) {
  ...
  try {
    ...
    // Enforce 5MB log rotation limit
    if (fs.existsSync(resolvedPath)) {
      const stats = fs.statSync(resolvedPath);
      if (stats.size > 5 * 1024 * 1024) { // 5MB limit
        for (let i = 2; i >= 1; i--) {
          const oldPath = `${resolvedPath}.${i}`;
          const newPath = `${resolvedPath}.${i + 1}`;
          if (fs.existsSync(oldPath)) {
            try { fs.renameSync(oldPath, newPath); } catch (_) {}
          }
        }
        try { fs.renameSync(resolvedPath, `${resolvedPath}.1`); } catch (_) {}
      }
    }
    fs.appendFileSync(resolvedPath, entry + '\n', 'utf8');
  } catch (err) {
    // Fail silently to prevent logger errors from crashing the main Node thread
  }
}
